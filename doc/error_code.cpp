...
public:
    Ret(); /* создаёт Val по-умолчанию.(Почему Val, а не ErrN?
              Может стоит убрать этот конструктор?)
                     Ничего не инициализировать нельзя. Некоторые места программы закладываются
                     на идею, что в Ret<> что-то есть.
           */
    Ret(const Val& v); /* убрал explicit, потому что писать return Ret<Val>(v); неудобно.
                          Но может есть причина, чтобы вернуть */
    Ret(Val&& v) noexcept; /* сравнить с конструктором копирования для Val.
...
/* видимо ещё и свой any писать придётся :(
   ...
   У меня не получилось передать в boost::any std::unique_ptr(из отсутствия copy constructor'а).
      Может я где-то ошибся? Надо перепроверить.
   ...
   hash -- хеш имени ti->name(). Вначале сравнивать с ним.
      Вообще-то странная оптимизация: если компилятор превращает сравнение с type_info в strcmp по имени,
        и следовательно уверен в неизменности имени, то почему сам не делает такой оптимизации?
...
// можно сделать pretty print для деструктора и операторов присваивания:
//   те выводить не сам тип, а типы содержимого Val и Set(поэлементно).

/*
  Ret<N, ErrA> v = ...

  if_err<ErrA>(v, [](ErrA&&) {}); // Считает, что раз в N ничего быть не может, то что-то в ErrA
  if_err<ErrA>(v, [](ErrA&&) {}); // Тоже самое, но в ErrA уже ничего нет!

  Проблему можно решить, если в Ret<Val, ...> будет флаг, который говорит, что он уже был
  в if_err(в любом; удачно/неудачно неважно) -- будет такая имитация линейных типов.

  Можно ли сымитировать такой флаг с помощью empty у any?
    Если исходить из идеи, что в "нормальном"(те таком, который можно засунуть в if_err
    или repack) Ret должно, что-то лежать, то empty -- это ненормально.
  
  Можно ли запретить move assign у Ret<>?
    Не думаю, в некоторых алгоритмах нужно вернуть ответ наверх(например
      мы что-то считаем в цикле и нужно вытянуть ответ из цикла).
         Кстати, хорошая причина почему Ret<Val, Errors> НЕ должен инициализировать
         Val, а быть empty.

  Пусть тут будет список проблем моего решения :( :
      1) двойной if_err на одном Ret<>  -- возможно лечится флагом
      2) присваивание Ret<> в котором что-то есть -- лечится проверкой empty
  Много чего переносится на run-time -- очень плохо. Вообще можно написать парсер, который
  бы проверял, что один и тот же Ret используют в if_err несколько раз, но даже если не считать
  обычных указателей и ссылок, возможность переиспользования Ret уже вызвает проблемы(а переиспользование
  нужно например для циклов(я где-то про это писал).

  Что если вообще не проверять двойное использование if_err? Просто не делать оптимизации расчитывая на тип?
  Учитывая, что от двойной проверки вреда никакого, а более низкая скорость -- это не страшно(тот кому нужна
  скорость будет использовать исключения), то это не плохой вариант. Даже проверять на empty в if_err не нужно
  так как там итак проверяется совпадение типа.
    Тогда что делать с присваиванием? Не заставлять же писать код в CPS. Было бы классно, если for и if
    возвращали значение, сейчас это реально.
    Учитывая всё выше, курс такой: 1) Если написано Ret<Val, Errors>, то это может значить: что там Val,
    что там Errors, и что там ничего нет => убрать оптимизации полагающие, что там всегда что-то есть
    (оптимизацию полагающую, что N там нет, можно оставить -- убрать один ==typeid).
    Для Ret<Val, Set<>> можно считать, что там Val есть(разумеется если это не N).
    2) запретить присваивание Ret<...> как только с Val, так и с Val,Errors; и move и copy.
    3) Так как конструктор по-умолчанию в Ret<...> освободился, то пусть делает Val по-умолчанию.
    В Ret<N, ...> это значение -- empty.

  Представим, что if_err<тут все ошибки v>(v, ...) перебрала все ошибки и проверила их, потом проверила
    на Val и это не Val; тогда она проверила на empty -- вопросы:
      1) может ли empty быть false?
      2) что вернуть если empty = false?
      3) что вернуть если empty = true?
         1) думаю, если empty = false, то это критическая ошибка(неизвестный тип в Any)
         2) кинуть исключение или как-то по-другому громко сказать
         3) если empty = true, то видимо тут двойной if_err на v.
             Двойной if_err -- критическая ошибка, просто одну
             if_err могли передать по указателю в разные функции
             и в одной всё уже извлекли(в том числе Val). Тогда у второй if_err ждут нормальных
             значений, а там -- каша. 
               Так что критическая ошибка -- завершаем программу. (напомню, что у деструктора Ret есть
                 оправдание: если там завершать программу с критической ошибкой, то нельзя будет
                 нормально кинуть исключение. Но в данном случае такого оправдания нет.
                    А действительно ли нормально кидать исключения при недопроверенном Ret<>?
                    Думаю что в Ret<> тоже критическая ошибка.)

  Вообще проверку на empty лучше делать у самого начала if_err. Это привносит некоторую неэффективность
  (двойной if_err делают не часто) зато можно проверять случаи когда проверяют не все ошибки, а хоть
  одну. А это значит что проверка будет ближе к месту ошибки и её легче будет найти отладчиком.
  Для скорости можно сделать эту проверку опциональной.(кстати, в таком случае нельзя будет проверять,
  что empty = true(те находить тип которого не должно быть), но я думаю, что такая ошибка маловероятна
  (ведь это значит, что ошибка у меня ^_^)).

*/  
    
/*
  Слабое место моего кода:
    Ret<Val, ErrA> v = ...
    if(очень редко) {
      if_err<ErrA>(v, ...);
    }
    ...
    if_err<ErrA>(v, ...);
  Лучше ли чем исключения?
    Если двойной if_err не ошибка, то лучше(imho).

  Другое:
    Ret<Val, ErrA, ErrB> v = ...
    if_err<ErrA>(v, ....);
  Val и ErrA бывают часто, а ErrB очень редко. Но мы проверяем только ErrA.
  Когда выпадет ErrB, то появится запись в логах, но это будет не скоро.
    А вот и нет: if_err вернёт Ret<Val, ErrB> и придётся проверять ErrB.
  ...
  Как переписать это?:
    ...
    while(1) {
      Ret<std::string, Err> ret = getline();

      Ret<std::string> str = if_err<Err>(ret, [](Err&) { (1) });

      sum += strlen(str.data());
    }
  Что написать в (1)? Можно throw; можно сделать if(stop)break, и протащить stop через замыкание
    в хендлер; возвращать пустую строку нельзя, так как это нормальное поведение getline, а не
    ошибочное; 
    ----------
    try {
      while(1) {
        ...
        Ret<std::string> str = if_err<Err>(ret, [](Err&) { throw Err; });
  ...
  Выглядит плохо :(
    ...
    while(1) {
      ...
      bool stop = false;
      Ret<std::string> str = if_err<Err>(ret, [&stop](Err&) { stop = true; });
      if(stop)
        break;
  ...
  Тоже выглядит плохо.
    ----------
    auto loop = [](int sum = 0) -> Ret<int, Err> {
      Ret<std::string, Err> ret = getline();
      Ret<int, Err> res = repack<int>(ret, [&sum, &loop](std::string&& str) { return loop(sum + strlen(str)); });
      return res;
    }
  Выглядит намного лучше, но как с эффективностью? Есть ли оптимизация хвостовой рекурсии?
    Скорее всего оптимизации нет из-за AutoClearAny.
...
/* Интересно есть либы для деманглинга type_info->name()? Если взять популярные компиляторы,
   узнать что они возвращают в type_info->name(), найти общие элементы и сделать либу для
   деманглинга. Возможно ли? Можно ли идетифицировать с помощью результата одинаковые типы?
   А если сделать конвертер для классов, то можно сделать либу intercompiler ^_^, только msvc со
   своим seh всё испортит, но вместо исключений можно error_handling.
*/

/* алгоритм получения типа для if_err.
   1) получить самый строгий тип(отбросить из Ret<> на входе все проверяемые ошибки)
   2) обойти все UnOps и сложить возвращаемые ими ошибки; Val у всех должен приводится к Val изначального Ret
   3) проверить, что строгий тип можно привести к полученному в 2 нестрогому(те в 2 есть все ошибки из 1)

   из 3) следует, что в 2) есть все ошибки из 1. Те если мы прибавим к 2-му все ошибки из 1, то 3 будет выполнятся.
*/

/* Нельзя делать is_err(Ret<>) -> bool, is_val(Ret<>) -> bool. Они позволяют делать сложные выражения
   с использованием if, а у меня слишком много делается в runtime. Если бы были линейные типы :( то
   можно было и сделать.
*/

/* Насчёт двойного if_err на один Ret<...>.

   Что если получение empty Ret<> в if_err не считается ошибкой(он просто его возвращает),
   а вот копирование или перемещение в Ret<> это уже ошибка.

   Тогда:
     Ret<Val, Err> v <- empty
     if_err<Err>(v, [](Err&&){}); // не ошибка
     Ret<Val> = if_err<Err>(v, [](Err&&){}); // ошибка
   А как обозначить, что в Ret<Val> лежит empty? Там же только Val.
     Можно сделать ошибкой в if_err, только если строгий тип это Ret<Val>.

   Так как if_err всё равно пишут с инициализацией Ret<...>, то от этого
     метода мало проку(только портит имитацию линейных типов неоднородностью)

   ---------------------------------------

   Вообще, если из Ret<...> не перемещать, а копировать, а empty для деструктора
   делать флагом, то проблемы двойного if_err уже нет. Но это дорого.

   Вообще можно сделать с помощью подписчиков. Когда делают Ret<...> a = Ret<...> b;,
   то присваивания не происходит: 'a' добавляет себя в 'b'.
   Если у 'a' вызывается деструктор и он всё ещё подписан в 'b', то он выписывает себя
   из 'b'. Если у 'b' вызывается деструктор, то он копирует/перемещает своё содержимое
   в подписчиков и сообщает им, что они выписаны.
     +) быстро с большими объектами
     -) медленно с маленькими(но маленькие можно копировать, и значит минуса нет)
     -) список подписчиков ест память
*/

/* надо написать тут пример, почему нельзя разрешать присваивание Ret<...>.

   Вообще "за" присваивание есть один огромный аргумент: с++ императивный язык,
   тут любят присваивать. Должна быть трудноуловимая ошибка с участием присваивания, но
   которые нельзя повторить с помощью repack и if_err.

   "Против" присваивания -- исчезает значение, причём мы не знаем какое Val или Err.

   Вообще: 1) если разрешить присваивание; 2) инициализация Ret<...> не empty(имеются ввиду конструкторы).
   Если присвоить Ret<...> empty, то будет такая же ошибка, как и в случае конструирования.

   Пример:
     Ret<Val, ErrA, ErrB> v = ...;
     Ret<Val, ErrB> r = if_err<ErrA>(v, ...);

     Ret<Val, ErrA, ErrB> v2 = v; // ERR
   Этот пример аналогичен двойному if_err или конструированию Ret<...> v2(v);

   Вообще: везде где можно присвоить к Ret<...>, можно и сконструировать Ret<...>.
      Есть отличие:
        Ret<Val, ErrA, ErrB> v = ...;

        if(редкое событие) {
          v = empty;
          /\* или auto z = if_err<ErrA>(v, ...); *\/
        }

        Ret<Val, ErrB> r = if_err<ErrA>(v, ...); // зависит от if
      но это отличие заменяется if_err.

   Итак: присваивать можно. Что насчёт того, что инициализация не должна быть empty?

     Синтаксис: конструктор по-умолчанию Ret<...>();

     -) у Ret<Val> нет состояния empty. Следовательно -- неоднородность.
           (у Ret<Val> уже есть неоднородность: метод data.)
     -) empty можно возвращать из функций: return Ret<...>();.
           (Нельзя вернуть, так как присваивание empty -- критическа ошибка)

   Поискать по тексту выше, и если ничего не нашёл, то разрешить присваивание и
   Ret<Val, Err>() инициализируется empty, а Ret<Val>() по-умолчанию.
   Деструктор, конструктор, if_err и присваивание -- критическая ошибка.

   Ещё вопрос: можно присваивать только к empty или всем?
     Вообще, я не знаю случая когда переменной с данными нельзя было
     присваивать, если она не empty: vector, unique_ptr, shared_ptr,
     всем можно присваивать. Наверное и для Ret<...> будет ОК.

   Вообще нужно рассматривать runtime проверки, как что-то вспомогательное.
       Их нормальное состояние -- отключено, а не включено(а критические они
       потому что не понятно что возвращать).
     Основная фишка, это список типов.
*/

/* Если в Ret описаны ошибки <ErrA, ErrB>, то мы не сможем сказать, что ErrC тоже ошибка.
   Но можно добавлять в список ошибок всё что возвращают функции, которые не возвращают:
   void; Ret<...>; что-то приводимое к Val.
 */

/* слабое место в Any. Конструкторы и деструкторы типов в union -- не инлайнятся.

   Возможно, что цепочка из:
     if(typeid(Val) == ...) { тут для Val }
     else if(typeid(ErrA) == ...) { ... }
     ....
   будет быстрее за счёт inline'а. Но если типов много, то может быть медленнее
   (но не думаю, что типов ошибок много).
      Хорошо работает с пустыми классами(там условия выкидываются).
 */

/* 14 july 2014 -- 100'000'000 iters: 1.67s / 0.35s ~= 4.5

   Проверялись итерационные фолды, которые считают сумму чисел.

   Причём тот, что с Ret<...> делал все проверки ошибок, а его опонент ничего не проверяет.
   В Ret<...> использовался Any на цепочках из if-typeid.

   Помоему очень не плохо: полезные вычисления играли небольшую роль, а итераций(а значит аллокаций)
   было много. В случае наличия более полезной нагрузки разница будет значительно меньше.

   ------------------------------
   Таже дата -- 100'000 iters: 2.16s / 2.15s ~= 1

   Фактически тот же тест, что и выше, но вместо сложения чисел была их конкатенация.

   Строковые операции съели всё время и поэтому время практически одинаковое.

   ------------------------------
   Первый тест после замены TypeInfo на type_info: 0.63 / 0.35 ~= 1.8
...
/* 16 july 2014
   ...
   Сделал в тесте с суммированием чисел reserve для vector'а. Теперь он вносит меньше погрешности.

   100'000'000 iters: 0.55s / 0.27s ~= 2.03

   Надо увеличивать итерации :/

   ------------------------------
   Увеличил в 2 раза: 200000000.

   Теперь: 1.09s / 0.59s ~= 1.84

   Думаю потери из-за нехилого strcmp. Тут ещё имя типа короткое, стоит увеличить и всё.

   ------------------------------
   С более длинным именем типа получилось 1.25s / 0.75s ~= 1.66
   Те длина имени типа не так сильно влияет

   ------------------------------
   Заменил генерацию вектора на простой счётчик чисел.
   Самое точное 5.28s. Разрыв примерно в 5 раз.

   ------------------------------
   Заменил создание через конструктор передачей по ссылке с присваиванием(это в if_err, repack,...).
