...
/*
    ...
    на Val и это не Val; тогда она проверила на empty -- вопросы:
      ...
      3) что вернуть если empty = true?
         ...
         3) если empty = true, то видимо тут двойной if_err на v.
             ...
             значений, а там -- каша. 
             Что можно вернуть? Раз перебрали все ошибки, то возвращают Ret<Val, Set<>>.
               А что тут можно изменить? Просто печатаем сообщение и возвращаем Val по-умолчанию.
  ...
  (ведь это значит, что ошибка у меня ^_^)).
     Только опять не понятно, что возвращать из if_err. Пусть будет критическая ошибка
